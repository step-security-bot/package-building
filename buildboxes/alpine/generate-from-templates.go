...
type JsonConfig struct {
	AlpineVersions []string `json:"AlpineVersions"`
}
...
func main() {
	...
	for i := range config.AlpineVersions {
		alpineVersion := config.AlpineVersions[i]

		// Create the Dockerfile for the specific Java version.
		dockerfileFile, err := os.Create(fmt.Sprintf("Dockerfile.v%s", alpineVersion))
		if err != nil {
			log.Fatal(err)
		}
